...
2. **Traversal** (`traverse` inner function)
   ...
   - For **text nodes**:
     ...
     - Normalizes using `normalizeText(text, { trim: false })`:
       ...
       - Does *not* trim at ends (because block‑level trimming happens later).
...
3. **Block normalization** (`normalizeBlocks`)
   ...
   - For each block:
     - Iterates over characters and collapses whitespace similar to frontend behavior:
       ...
       - Leading whitespace before any non‑whitespace in the *entire output* is dropped.
...
1. **Tree walker setup**
   ...
   - `acceptNode` logic:
     ...
     - **Anchors outside `<p>` are *not* specially skipped**:
...
- For consecutive blocks `currentBlock` and `nextBlock`:
  ...
  - If the tag at that index is in `BLOCK_TAGS` for either block, that boundary is considered a **potential structural break**.
- Headers:
...
- The reported `start` and `end` indices:
  - Refer to positions in the **concatenated, normalized text**, *excluding* leading/trailing whitespace of the sentence itself.
