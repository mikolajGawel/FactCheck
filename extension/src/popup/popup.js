...
async function loadArticles() {
    ...
    if (jobState === 1) { render.renderInProgress(); return; }
    if (jobState === 2) { render.renderCompleted(); return; }
    if (jobState === -1) { render.renderError("Ostatnie zadanie nie powiodło się."); return; }
    ...
    chrome.tabs.sendMessage(tab.id, { type: "getArticles" }, response => {
        ...
        render.renderArticles(articles, selectArticle);
    });
}
...
startBtn.addEventListener("click", async () => {
    ...
    const response = await new Promise(resolve => {
        chrome.runtime.sendMessage({ type: "setJobState", jobState: 1, tabId: tab.id }, resolve);
    });
    ...
    render.renderInProgressWithEstimate(article);
    chrome.tabs.sendMessage(tab.id, { type: "startJob", articleId: selectedId, title: article?.title ?? null, url: tab.url });
});
...
chrome.runtime.onMessage.addListener(async message => {
    ...
    switch (jobState) {
        case 1: render.renderInProgress(); break;
        case 2: render.renderCompleted(); break;
        case -1: render.renderError(message.error || "Błąd"); break;
...
async function fetchAndApplyState() {
    ...
    switch (jobState) {
        case 1: render.renderInProgress(); break;
        case 2: render.renderCompleted(); break;
        case -1: render.renderError("Ostatnie zadanie nie powiodło się."); break;
        default: loadArticles(); break;
    }
}
...
document.addEventListener("visibilitychange", () => { if (!document.hidden) fetchAndApplyState(); });
