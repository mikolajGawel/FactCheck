...
export function renderArticles(list, onSelect) {
    stopProgressAnimation(false, true);
    if (!articlesContainerEl) return;
    articlesContainerEl.innerHTML = "";
    if (!list || !list.length) {
        articlesContainerEl.innerHTML = '<div class="loading">Na stronie nie znaleziono elementu z artykułem: &lt;article&gt;.</div>';
...
export function renderInProgress({ forceIndeterminate = false } = {}) {
...
export function renderInProgressWithEstimate(article) {
    if (!articlesContainerEl || !startBtnEl || !titleTextEl) return;
